...
def update_table_meta(cptac):
  collstrt = cptac[0]
  #table = clinJson[collstrt]['srcs']['table']
  hist = {}
  table_id = DEFAULT_PROJECT + "." + LAST_DATASET + '.table_metadata'
  src_table_id = clinJson[collstrt]['srcs']['table']
...
def copy_cptca(coll):
  src_table_id = clinJson[coll]['srcs']['table']
  client = bigquery.Client()
  src_table = client.get_table(src_table_id)
...
if __name__=="__main__":
  ...
  update_table_meta(cptac)
  copy_cptca(cptac[0])
  update_column_meta(cptac)